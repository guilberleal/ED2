...
void inserir_aresta ( tipo_grafo *grafo, char vertice_1, char vertice_2, int tempo_de_retardo  ) {
   if ( grafo->contador_vertices > 1 ) {
      ...
      if ( u == infinito || v == infinito )
         if ( u == infinito ) printf ( "[ erro ] o vértice %c não existe.\n", vertice_1 );
         if ( v == infinito ) printf ( "[ erro ] o vértice %c não existe.\n", vertice_2 );
      else {
...
int eh_adjacente ( tipo_grafo *grafo, int posicao1, int posicao2 ) {
	if ( grafo->matriz_adjacencia[posicao1][posicao2] != infinito ) return 1;
	else return 0;
}
